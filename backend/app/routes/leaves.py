...
# Actions that mean "may see any employee's leave" (aligned with get_hierarchical_permissions + frontend view_all)
...
async def leave_open_allowed_for_user(
    ...
    # Fallback: set overlap (handles BSON type skew where find_one missed but list_leaves still matched)
...
async def enrich_leave_data(
    ...
    # No approvers configured → inject superadmins as default approvers (display only)
...
async def _get_superadmin_user_ids(users_db: UsersDB, roles_db: RolesDB) -> List[str]:
    ...
    Super-admin is detected via three strategies (in order):
    1. Role has permissions with page='*' and actions='*'  (most common)
    2. Role document has is_super_admin=True flag
    3. User document has is_super_admin=True flag
    """
    result: List[str] = []
    seen: set = set()

    # Strategy 1: roles whose permissions include {"page": "*", "actions": "*"}
    super_role_ids: List[str] = []
    async for role in roles_db.collection.find(
        {"permissions": {"$elemMatch": {"page": "*", "actions": "*"}}},
        {"_id": 1},
    ):
        super_role_ids.append(str(role["_id"]))

    # Strategy 2: roles with is_super_admin=True flag
    ...
    # Strategy 3: users with is_super_admin=True on user document
...
async def create_leave(
    ...
    try:
        ...
        is_default_approval = False  # True = any-one-approval rule, False = all-must-approve

        # Step 2: no approver sent → look up settings for the employee's role
        if not approver_ids:
        ...
        # Step 3: still no approver → use superadmin(s) as default (any-one rule)
        if not approver_ids:
            approver_ids = await _get_superadmin_user_ids(users_db, roles_db)
            is_default_approval = True

        # Build approvers array with per-approver status
        approvers_list = []
        ...
        # Enrich with user details (pass roles_db so superadmin is shown when no approver set)
...
async def list_leaves(
    ...
    try:
        ...
        # Build filter: same Mongo employee_id rule as GET /leaves/{id} (detail is never stricter than list)
        filter_dict = {}
...
async def get_leave(
    ...
    try:
        ...
        # ── Permission: mirror list_leaves exactly ──────────────────────────
        # Build the same employee_id filter the list uses, then do a find_one
        # with the leave's _id added.  If the document is returned the user
        # would have seen it in the list → allow the detail popup.
        ctx = await resolve_leave_access_scope(current_user_id, users_db, roles_db)
        access_ok = False
        ...
        # Pass roles_db so superadmins are shown when no approver is configured
        leave = await enrich_leave_data(leave, users_db, roles_db)
        leave["id"] = leave["_id"]
        return leave
...
async def approve_reject_leave(
    ...
    try:
        ...
        if approvers:
            ...
            # is_default=True on ANY approver entry → "any-one" rule (superadmin fallback)
            # Otherwise → "all-must-approve" rule (configured approvers)
            is_default_leave = any(ap.get("is_default") for ap in approvers)

            all_statuses = [ap.get("status") for ap in approvers]
            acted_statuses = [s for s in all_statuses if s != "pending"]

            if is_default_leave:
                # Any-one rule: first approval/rejection decides the leave
            ...
            # No approvers array → check if current user is superadmin or has approve permission
            ctx = await resolve_leave_access_scope(current_user_id, users_db, roles_db)
            ...
            # Build this approver's entry and save into the approvers array
            ...
            new_approver_entry = {
                ...
                "is_default": True,  # legacy leaves treated as any-one rule
            }
            update_data: Dict[str, Any] = {
                "approvers": [new_approver_entry],
                "status": approval_data.status,  # any-one rule: immediate decision
                "updated_at": now,
            }
            if approval_data.status == "approved":
                update_data["approved_by"] = current_user_id
