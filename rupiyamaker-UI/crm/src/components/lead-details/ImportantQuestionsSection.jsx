...
export default function ImportantQuestionsSection({ leadData, lead, onUpdate, currentUserRole }) {
    ...
    useEffect(() => {
        ...
        // Properly load responses from leadInfo
        if (leadInfo?.question_responses) {
            console.log('Loading question responses from leadInfo:', leadInfo.question_responses);
            
            // Create a clean object with boolean values
            const cleanedResponses = {};
            Object.keys(leadInfo.question_responses).forEach(key => {
                const value = leadInfo.question_responses[key];
        ...
        } else if (leadInfo?.importantquestion) {
            // Handle importantquestion format
            const convertedResponses = {};
            Object.keys(leadInfo.importantquestion).forEach(key => {
                const value = leadInfo.importantquestion[key];
        ...
        } else if (leadInfo?.dynamic_fields?.important_questions) {
            // Convert legacy format to new format if needed
            const legacyResponses = leadInfo.dynamic_fields.important_questions;
            const convertedResponses = {};
    ...
    const handleQuestionResponse = async (questionId, checked) => {
        ...
        // Make sure leadInfo exists before proceeding with API call
        if (!leadInfo) {
            console.error("Cannot update question: leadInfo is undefined");
            return;
        }
        ...
        window.importantQuestionsTimeout = setTimeout(async () => {
            try {
                ...
                if (allMandatoryAnswered) {
                    ...
                    apiResponse = await fetch(`${API_BASE_URL}/lead-login/validate-questions/${leadInfo._id}?user_id=${userId}`, {
                        method: 'PATCH',
                        headers: {
                            'Authorization': `Bearer ${localStorage.getItem('token')}`,
                            'Content-Type': 'application/json'
                    ...
                    apiResponse = await fetch(`${API_BASE_URL}/leads/${leadInfo._id}?user_id=${userId}`, {
                        method: 'PUT',
                        headers: {
                            'Authorization': `Bearer ${localStorage.getItem('token')}`,
                            'Content-Type': 'application/json'
    ...
    const shouldShowSendButton = () => {
        return (
            ...
            leadInfo?.sub_status === 'FILE COMPLETED' &&
            !leadInfo?.file_sent_to_login &&
            questions.length > 0 &&
    ...
    const handleSendToLoginDepartment = async () => {
        ...
        setIsSending(true);
        try {
            const response = await fetch(`${API_BASE_URL}/lead-login/send-to-login-department/${leadInfo._id}?user_id=${userId}`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
                    'Content-Type': 'application/json'
        ...
        <div className="mb-4 border border-gray-700 rounded-lg bg-white">
            ...
            >
                <span className="flex items-center">
                    <CheckCircle className="w-6 h-6 mr-2 text-[#03b0f5]" />
                    Important Questions
                </span>
                                ...
                                <div className="mt-4 p-3 rounded-lg border border-gray-300 bg-gray-100">
                                    {areAllMandatoryQuestionsAnswered() ? (
                                        <div className="flex items-center text-green-700">
                                            ...
                                            {leadInfo?.sub_status === 'FILE COMPLETED' && !leadInfo?.file_sent_to_login && (
