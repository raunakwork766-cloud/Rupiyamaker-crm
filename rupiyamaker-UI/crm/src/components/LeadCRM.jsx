                    ...
                    content: (
                        <div>
                            <LazySection height="350px">
                                <Attachments leadId={lead._id} userId={localStorage.getItem('userId') || localStorage.getItem('user_id')} lead={lead} />
                            </LazySection>
                        </div>
                    ),
                },
    ...
    if (selectedLead) {
        ...
        return (
            <div className="h-[calc(100dvh-3.5rem)] sm:h-[calc(100dvh-4rem)] overflow-hidden bg-black text-white text-base flex flex-col" style={activeTab === 1 ? {fontSize:'90%'} : {}}>
                ...
                {selectedLead && (
                    <div className="px-2 sm:px-4 lg:px-7 py-2 bg-[#0a1525] border-b border-[#1a2332] w-full">
                        <h2 className="text-base sm:text-lg font-bold text-white tracking-wide truncate">
                            {(`${selectedLead.first_name || ''} ${selectedLead.last_name || ''}`.trim()) || selectedLead.customerName || selectedLead.name || 'Lead'}
                        </h2>
                    </div>
                )}

                {/* Tabs */}
                <div className="flex flex-wrap items-center gap-1.5 px-2 sm:px-4 lg:px-7 py-1.5 bg-black border-b border-[#232c3a] w-full overflow-x-auto">
                    {detailSections.map((tab, idx) => (
                        <button
                            key={tab.label}
                            className={`
                                flex items-center px-3 sm:px-4 lg:px-5 py-1 sm:py-1.5 rounded-3xl font-extrabold border shadow-md text-xs sm:text-sm lg:text-[0.9rem] transition whitespace-nowrap
                                ${idx === activeTab
