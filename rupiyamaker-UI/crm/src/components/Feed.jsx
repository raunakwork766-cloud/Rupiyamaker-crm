...
const isFeedAllowedUpload = (file) =>
    isFeedImageFile(file) || file.type === "application/pdf";
    ...
    files
        .filter((f) => {
            ...
            return (
                ...
                /\.(jpe?g|png|gif|webp|bmp|heic|heif)$/i.test(filePath)
            );
        })
        .map((f) => buildMediaUrl(f.file_path));

const feedImageClassName =
    "block w-auto h-auto max-w-full max-h-[354px] sm:max-h-[430px] object-contain";
...
function PollDisplay({ poll, postId, currentUserId, onPollUpdate }) {
    ...
    const initIdx = () =>
        (poll?.options || []).findIndex((o) =>
            (o.voters || []).includes(currentUserId ?? ""),
        );

    // 'idle' | 'voted' | 'open'
    const [phase, setPhase] = React.useState(() =>
        initIdx() >= 0 ? "voted" : "idle",
    );
    const [myIdx, setMyIdx] = React.useState(() => {
        const i = initIdx();
        return i >= 0 ? i : null;
    });
        ...
        if (curPhase === "idle" || curPhase === "open") {
            ...
            try {
                ...
                if (res.ok && data?.poll?.options) {
                    ...
                    // revert
                    setCounts((p) =>
                        p.map((c, x) => (x === i ? Math.max(0, c - 1) : c)),
                    );
                    const rollback = curPhase;
                    setPhase(rollback);
                    setMyIdx(curIdx);
            ...
            try {
                const res = await fetch(
                    buildApiUrl(
                        `feeds/${postId}/poll/unvote?user_id=${encodeURIComponent(currentUserId)}`,
                    ),
                    {
                        method: "POST",
                        headers: {
    ...
    return (
        <div className="mx-4 my-3 border border-gray-200 rounded-xl p-4 bg-gray-50">
            <p className="font-semibold text-gray-900 text-[15px] mb-3">
                {poll.question}
            </p>
                ...
                {(poll.options || []).map((opt, i) => {
                    ...
                    return (
                        ...
                        >
                            {showBars && (
                                <div
                                    className={`absolute left-0 top-0 h-full rounded-lg transition-all duration-700 ${isMe ? "bg-blue-100" : "bg-gray-100"}`}
                                    style={{ width: `${pct}%` }}
                                />
                            )}
                            <span
                                className={`relative z-10 text-sm font-medium flex-1 ${isMe ? "text-blue-700" : "text-gray-700"}`}
                            >
                                {isMe && "\u2713 "}
                                {opt.text}
                            </span>
                            {showBars && (
                                <span className="relative z-10 text-xs text-gray-500 font-semibold ml-2">
                                    {pct}%
                                </span>
                            )}
                        </button>
...
function flattenComments(comments, parentAuthor = null, level = 0) {
    ...
    comments.forEach((comment) => {
        ...
        if (comment.replies && comment.replies.length > 0) {
            result.push(
                ...flattenComments(comment.replies, comment.author, level + 1),
            );
        }
    });
    return result;
            ...
            {sortedComments.map((comment) => (
                <React.Fragment key={comment.id}>
                    <div className="flex items-start space-x-2 sm:space-x-3 mt-2 sm:mt-3">
                        <div className="w-6 sm:w-8 h-6 sm:h-8 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full overflow-hidden flex items-center justify-center flex-shrink-0">
                            ...
                            <span
                                ...
                                style={{
                                    display: comment.profilePicture
                                        ? "none"
                                        : "flex",
                                }}
                            >
                        ...
                        <div className="flex-1">
                            <div className="bg-white rounded-2xl px-2 sm:px-3 py-2 border border-gray-700">
                                ...
                                <div className="text-black mt-1 text-sm sm:text-base">
                                    {comment.text}
                                </div>
                                <div className="flex gap-2 sm:gap-4 mt-1 text-xs">
                                    ...
                                    >
                                        <ThumbsUp
                                            className={`w-3 sm:w-4 h-3 sm:h-4 transition-all duration-300 ${comment.liked === true ? "fill-blue-600 scale-110" : "fill-none"}`}
                                        />
                                ...
                                {replyBoxState &&
                                    ...
                                    replyBoxState.commentId === comment.id && (
                                        <div className="flex items-start space-x-2 sm:space-x-3 mt-1">
                                            <div className="w-6 sm:w-8 h-6 sm:h-8 bg-gradient-to-r from-purple-600 to-blue-600 rounded-full overflow-hidden flex items-center justify-center flex-shrink-0">
                                                {currentUserPhoto ? (
                                                    <img
                                ...
                                {[...comment.replies]
                                    ...
                                    .map((reply) => (
                                        ...
                                        >
                                            ...
                                            <div className="w-6 sm:w-8 h-6 sm:h-8 bg-gradient-to-br from-blue-400 to-blue-700 rounded-full overflow-hidden flex items-center justify-center flex-shrink-0">
                                            ...
                                            <div className="flex-1">
                                                <div className="bg-blue-50 rounded-2xl px-2 sm:px-3 py-2 border border-blue-300">
                                                    ...
                                                    <div className="flex gap-2 sm:gap-4 mt-1 text-xs">
                                                        ...
                                                        >
                                                            <ThumbsUp
                                                                className={`w-3 sm:w-4 h-3 sm:h-4 transition-all duration-300 ${reply.liked === true ? "fill-blue-600 scale-110" : "fill-none"}`}
                                                            />
                                                            <span className="font-semibold">
                                                                {reply.likeCount ||
                                                                    0}
            ...
            {feed.images && feed.images.length > 0 && (
                ...
                >
                    {feed.images.slice(0, 4).map((image, index) => (
                        <div
                            key={index}
                            className={`relative ${feed.images.length === 1 ? "" : ""}`}
                        >
                            <img
                                src={image}
                                ...
                                className={`w-full cursor-pointer hover:opacity-90 transition-opacity ${
                                    feed.images.length === 1
                                        ? "h-[500px] sm:h-[600px] object-contain"
                                        : "h-80 sm:h-96 object-cover"
                                }`}
                                onClick={() =>
                                    onOpenImageViewer(feed.images, index)
                                }
                            />
            ...
            <div className="px-3 sm:px-4 py-2 sm:py-3 border-t border-gray-800">
                <div className="flex items-center justify-between">
                    <div className="flex space-x-3 sm:space-x-6 w-full">
                        ...
                        >
                            <ThumbsUp
                                className={`w-4 sm:w-5 h-4 sm:h-5 transition-all duration-300 ${feed.liked ? "fill-blue-600 scale-110" : "fill-none"}`}
                            />
...
export default function FeedPage({ user }) {
    ...
    const canViewFeeds =
        hasPermission(userPermissions, "feeds", "show") ||
        hasWildcardPermission;
    const canCreatePost =
        hasPermission(userPermissions, "feeds", "post") ||
        hasWildcardPermission;
    ...
    const currentUserPhoto = (() => {
        try {
            ...
            if (photo) return getProfilePictureUrlWithCacheBusting(photo);
            const ud = JSON.parse(localStorage.getItem("userData") || "{}");
            return ud.profile_photo
                ? getProfilePictureUrlWithCacheBusting(ud.profile_photo)
                : null;
        } catch {
            return null;
        }
    ...
    const loadFeeds = useCallback(async () => {
        try {
            ...
            const storedLikes = JSON.parse(
                localStorage.getItem(commentLikesKey) || "{}",
            );
            ...
            const transformedFeeds = (data.items || []).map((feed) => {
                ...
                try {
                    if (feed.created_at)
                        timestamp = formatDate(feed.created_at);
                } catch (e) {}
                ...
                if (feed.comments) {
                    ...
                    const allComments = feed.comments.map((c) => {
                        ...
                        try {
                            if (c.created_at)
                                commentTimestamp = formatDate(c.created_at);
                        } catch (e) {}

                        const isLikedByUser =
                            storedLikes[c.id || c._id] || false;
                ...
                return {
                    ...
                    author_photo: feed.creator_photo
                        ? getProfilePictureUrlWithCacheBusting(
                              feed.creator_photo,
                          )
                        : null,
    ...
    const loadCommentsForFeed = useCallback(async (feedId) => {
        try {
            ...
            const response = await fetch(
                buildApiUrl(
                    `feeds/${feedId}/comments?user_id=${encodeURIComponent(currentUserId)}`,
                ),
                {
                    method: "GET",
                    headers: {
    ...
    const handlePost = async () => {
        ...
        if (showPollCreator) {
            ...
            if (selectedFiles.length > 0 && message.trim() === "") {
                setMessageValidationError(
                    "A message is required when posting photos or files.",
                );
                return;
            }
        }
        ...
        try {
            ...
            if (showPollCreator) {
                ...
                const response = await fetch(
                    buildApiUrl(
                        `feeds/poll?user_id=${encodeURIComponent(currentUserId)}`,
                    ),
                    {
                        method: "POST",
                        headers: {
                ...
                selectedFiles.forEach((fileObj) => {
                    formData.append(
                        "files",
                        fileObj.file,
                        fileObj.file.name || fileObj.name || "upload",
                    );
                });
                const response = await fetch(
                    buildApiUrl(
                        `feeds/?user_id=${encodeURIComponent(currentUserId)}`,
                    ),
                    {
                        method: "POST",
                        headers: {
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 504) {
                        throw new Error(
                            "Upload timed out. Try a smaller file or retry in a moment.",
                        );
                    }
    ...
    const handleComment = useCallback(
        async (feedId, commentText) => {
            if (!commentText.trim()) return;
            try {
                // Create temporary comment for immediate UI update
                const tempId = `temp-${Date.now()}`;
                const _currentUserData = JSON.parse(
                    localStorage.getItem("userData") || "{}",
                );
                const _currentUserPhoto = (() => {
                    const p =
                        localStorage.getItem("userProfilePhoto") ||
                        localStorage.getItem("profile_photo") ||
                        _currentUserData.profile_photo;
                    return p ? getProfilePictureUrlWithCacheBusting(p) : null;
                })();
                const newComment = {
                    id: tempId,
                    text: commentText,
                    author: userName,
                    authorId: userId,
                    profilePicture: _currentUserPhoto,
                ...
                setFeeds((prevFeeds) =>
                    prevFeeds.map((feed) =>
                        feed.id === feedId
                            ? {
                                  ...feed,
                                  comments: [newComment, ...feed.comments],
                              }
                            : feed,
                    ),
                );
                ...
                // NOTE: backend route is /feeds/{post_id}/comments (NO trailing slash).
                // The feeds router uses redirect_slashes=False, so a trailing slash here
                // triggers a 307 that becomes a 405 through the nginx proxy → "Failed to comment".
                const response = await fetch(
                    buildApiUrl(`feeds/${feedId}/comments`),
                    {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
    ...
    const handleLikeComment = useCallback(
        async (feedId, commentId, parentId = null) => {
    ...
    const handleReply = async (feedId) => {
        ...
        try {
            ...
            const _replyUserData = JSON.parse(
                localStorage.getItem("userData") || "{}",
            );
            const _replyUserPhoto = (() => {
                const p =
                    localStorage.getItem("userProfilePhoto") ||
                    localStorage.getItem("profile_photo") ||
                    _replyUserData.profile_photo;
                return p ? getProfilePictureUrlWithCacheBusting(p) : null;
            })();
            const newReply = {
                id: tempId,
                ...
                author: userName,
                authorId: userId,
                profilePicture: _replyUserPhoto,
                timestamp: formatDate(new Date(), true),
                canDelete: true,
                liked: false,
                likeCount: 0,
            ...
            // NOTE: backend route is /feeds/{post_id}/comments (NO trailing slash).
            // redirect_slashes=False on the feeds router → trailing slash causes 307→405.
            const response = await fetch(
                buildApiUrl(`feeds/${feedId}/comments`),
                {
                    method: "POST",
                    ...
                    const replaceReply = (comments) =>
                        comments.map((comment) => {
                            ...
                            if (comment.replies) {
                                return {
                                    ...comment,
                                    replies: replaceReply(comment.replies),
                                };
                            }
                            return comment;
                        });
                    ...
                    const removeTemp = (comments) =>
                        comments.map((comment) => {
                            ...
                            if (comment.replies) {
                                return {
                                    ...comment,
                                    replies: removeTemp(comment.replies),
                                };
                            }
                            return comment;
                        });
    ...
    const handleDeleteComment = async (feedId, commentId) => {
        ...
        try {
            ...
            const response = await fetch(
                buildApiUrl(
                    `feeds/comments/${commentId}/?user_id=${encodeURIComponent(currentUserId)}`,
                ),
                {
                    method: "DELETE",
                    headers: {
        ...
        return (
            <div className="flex flex-col items-center justify-center h-64">
                ...
                <p className="text-gray-400">
                    You don't have permission to view the feeds.
                </p>
            </div>
        );
    }
        ...
        <FeedPageLayout userId={userId} feeds={feeds}>
            <>
                <div className="w-full px-2 sm:px-4 pt-2 sm:pt-4 pb-4 sm:pb-8">
                    {canCreatePost && (
                        <div className="bg-white rounded-lg shadow-sm border border-gray-800 mb-4 sm:mb-6">
                            <div className="p-3 sm:p-4 border-b border-gray-800">
                                <div className="flex items-center space-x-2 sm:space-x-3">
                                    <div className="w-8 sm:w-10 h-8 sm:h-10 bg-gradient-to-r from-purple-600 to-blue-600 rounded-full overflow-hidden flex items-center justify-center flex-shrink-0">
                                        {currentUserPhoto ? (
                                            <img
                                                src={currentUserPhoto}
                                                alt={userName}
                                                        ...
                                                        >
                                                            ...
                                                            "image" ? (
                                                                ...
                                                                <div className="w-full h-24 sm:h-32 bg-gray-800 rounded-lg flex items-center justify-center">
                                                                    ...
                                                                    <span className="ml-1 sm:ml-2 text-xs sm:text-sm text-gray-300 truncate max-w-[60px] sm:max-w-[80px]">
                                                                        {
                                                                            fileObj.name
                                                                        }
                                                                    </span>
                        ...
                        {visibleFeeds.map((feed) => {
                            ...
                            const postForPermission = {
                                ...
                                created_by:
                                    ...
                                    feed.authorId ||
                                    feed.creator_id ||
                                    (feed.creator && feed.creator.id),
                            };
                            const userCanDeletePost = canDeletePost(
                                userPermissions,
                                postForPermission,
                                currentUserId,
                            );
                            ...
                            const authorName =
                                feed.author || feed.creator_name || "User";
                                    ...
                                    <div className="border-t border-gray-100 mx-1">
                                        <div className="flex items-center">
                                            {canLike && (
                                                ...
                                                >
                                                    <ThumbsUp
                                                        className={`w-[18px] h-[18px] transition-all ${feed.liked ? "fill-blue-600 text-blue-600 scale-110" : ""}`}
                                                    />
                                                    <span>
                                                        {feed.liked
                                                            ? "Liked"
                        ...
                        <div className="w-full lg:w-[380px] xl:w-[420px] flex-shrink-0 bg-white flex flex-col max-h-[58vh] lg:max-h-full border-t lg:border-t-0 lg:border-l border-gray-200">
                            ...
                            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 flex-shrink-0">
                                <div className="flex items-center gap-3 min-w-0">
                                    <div className="w-10 h-10 rounded-full flex-shrink-0 overflow-hidden bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center">
                                        {viewerFeed.author_photo ? (
                                            <img
                                                src={viewerFeed.author_photo}
                                                alt={viewerFeed.author}
                                                className="w-full h-full object-cover"
                                                onError={(e) => {
                                                    e.target.style.display =
                                                        "none";
                                        ...
                                        <span
                                            className="text-white text-sm font-bold"
                                            style={{
                                                display: viewerFeed.author_photo
                                                    ? "none"
                                                    : "flex",
                                            }}
                                        >
                                ...
                                <div className="flex items-center justify-between px-4 py-2 border-b border-gray-100 flex-shrink-0">
                                    ...
                                    {viewerFeed.comments?.length > 0 && (
                                        <span className="text-sm text-gray-500 ml-auto">
                                            {viewerFeed.comments.length} comment
                                            {viewerFeed.comments.length !== 1
                                                ? "s"
                                                : ""}
                                        </span>
                                    )}
                            ...
                            <div className="border-b border-gray-100 flex-shrink-0">
                                <div className="flex items-center px-1">
                                    {canLike && (
                                        ...
                                        >
                                            <ThumbsUp
                                                className={`w-[18px] h-[18px] ${viewerFeed.liked ? "fill-blue-600 text-blue-600" : ""}`}
                                            />
                                            {viewerFeed.liked
                                                ? "Liked"
                                                : "Like"}
                                    ...
                                    {canComment && (
                                        <button
                                            type="button"
                                            onClick={() =>
                                                viewerCommentInputRef.current?.focus()
                                            }
                            ...
                            {canComment && (
                                <div className="border-t border-gray-200 px-4 py-3 flex items-center gap-2 flex-shrink-0 bg-white">
                                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 overflow-hidden flex items-center justify-center flex-shrink-0">
                                        {currentUserPhoto ? (
                                            <img
                                                src={currentUserPhoto}
                                                alt={userName}
                ...
                {showPostModal && selectedPost && (
                    <div className="fixed inset-0 bg-transparent bg-opacity-50 flex items-center justify-center z-50 backdrop-blur-sm p-2 sm:p-4">
                        <div
                            className="bg-white rounded-lg w-full max-w-sm sm:max-w-3xl mx-2 sm:mx-4 max-h-[95vh] sm:max-h-[90vh] overflow-hidden border border-gray-800 shadow-2xl flex flex-col"
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between p-3 sm:p-4 border-b border-gray-800 bg-white sticky top-0 z-10">
                                <div className="flex items-center space-x-2 sm:space-x-3">
                                    <div className="w-8 sm:w-10 h-8 sm:h-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full overflow-hidden flex items-center justify-center flex-shrink-0">
                                        {selectedPost.author_photo ? (
                                            <img
                                                src={selectedPost.author_photo}
                            ...
                            <div className="overflow-y-auto flex-1">
                                ...
                                <div className="px-3 sm:px-4 py-2 sm:py-3">
                                    <p className="text-black whitespace-pre-wrap bg-white font-bold text-base sm:text-lg">
                                        {selectedPost.text}
                                    </p>
                                </div>
                                ...
                                <div
                                    id="comments-section"
                                    className="border-t border-gray-800 px-3 sm:px-4 py-2 sm:py-3 bg-white"
                                >
                                    <div className="font-bold text-black mb-2 text-sm sm:text-base">
                                        Comments
                                    </div>
...
function MyTasksWidget({ userId }) {
    ...
    useEffect(() => {
        ...
        fetch(
            `/api/tasks/?assigned_to=${encodeURIComponent(userId)}&user_id=${encodeURIComponent(userId)}`,
            {
    ...
    const counts = useMemo(() => {
        const ongoing = tasks.filter((t) =>
            ["pending", "in_progress", "assigned"].includes(
                (t.status || "").toLowerCase(),
            ),
        ).length;
        const assisting = tasks.filter(
            (t) => (t.role || "") === "assisting",
        ).length;
        const setByMe = tasks.filter(
            (t) =>
                String(t.created_by || t.creator_id) === String(userId) &&
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            {popular.length === 0 ? (
                ...
                <ul className="divide-y divide-gray-50">
                    {popular.map((post) => {
                        const authorName =
                            post.author || post.creator_name || "User";
                        const initials = authorName
                            .split(" ")
                            .map((n) => n[0])
                            .join("")
