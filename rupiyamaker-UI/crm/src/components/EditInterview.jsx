...
const ClockTimePicker = ({ value, onChange, onClose, isOpen, className = "" }) => {
  ...
  const handleOK = () => {
    ...
    console.log("🕐 ClockTimePicker handleOK called, timeString:", timeString);
    onChange(timeString);
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50" onClick={(e) => {
      e.preventDefault();
      e.stopPropagation();
      onClose();
    }}>
...
export default function EditInterview({ 
  ...
  // Sync interview state when initialInterview prop changes
  useEffect(() => {
    if (initialInterview) {
      console.log("🔄 EditInterview: initialInterview prop changed, updating local state", initialInterview);
      setInterview(formatInterviewForUI(initialInterview));
    }
  }, [initialInterview]);
  ...
  // Handle reschedule toggle
  const handleRescheduleToggle = (checked) => {
    console.log("🔄 Reschedule toggle:", checked, "Current interview data:", interview);
    setIsRescheduling(checked);
    
    // If enabling reschedule mode, populate with current interview data
    if (checked) {
      setRescheduleDate(interview.interview_date || '');
      setRescheduleTime(interview.interview_time || '');
      console.log("🔄 Populated reschedule form with:", { 
        date: interview.interview_date, 
        time: interview.interview_time 
      });
    } else {
      // Clear reschedule form when disabling
      setRescheduleDate('');
      setRescheduleTime('');
    }
  };
  ...
  const handleReschedule = async () => {
    ...
    try {
      ...
      // Update local interview state with the new data
      setInterview(prev => ({
        ...prev,
        interview_date: rescheduleDate, // Keep in YYYY-MM-DD format for the UI
        interview_time: rescheduleTime,
        status: 'rescheduled'
      }));
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      console.log("🔧 EditInterview calling onSave with:", { interviewId: interview.id, interviewData });
      await onSave(interview.id, interviewData);
      console.log("🔧 EditInterview onSave completed successfully");

      // Update local interview state with the new data to reflect changes
      setInterview(prev => ({
        ...prev,
        ...interview, // Keep current form values
      ...
      toast.success("Interview updated successfully! The table has been updated with the new information.");
      setIsOpen(false);
      onClose();
    } catch (err) {
            ...
            <div className="flex-1 relative">
              ...
              <div className="relative">
                ...
                <ClockTimePicker
                  value={interview.interview_time}
                  onChange={(time) => {
                    console.log("🕐 Main time picker onChange called with:", time);
                    handleInputChange("interview_time", time);
                    setShowMainTimePicker(false);
                  }}
                  onClose={() => {
                    console.log("🕐 Main time picker onClose called");
                    setShowMainTimePicker(false);
                  }}
          ...
          <div className="mt-6 p-4 border-2 border-orange-300 rounded-lg bg-orange-50">
            <div className="flex items-center gap-3 mb-4">
              <input
                ...
                onChange={(e) => handleRescheduleToggle(e.target.checked)}
                  ...
                  <div className="flex-1 relative">
                    ...
                    <div className="relative">
                      ...
                      <ClockTimePicker
                        value={rescheduleTime}
                        onChange={(time) => {
                          console.log("🕐 Reschedule time picker onChange called with:", time);
                          setRescheduleTime(time);
                          setShowRescheduleTimePicker(false);
                        }}
                        onClose={() => {
                          console.log("🕐 Reschedule time picker onClose called");
                          setShowRescheduleTimePicker(false);
                        }}
