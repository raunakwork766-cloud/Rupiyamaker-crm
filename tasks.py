...
async def create_task(
    ...
    await check_permission(user_id, "tasks", "create", users_db, roles_db)
...
async def list_tasks(
    ...
    visibility_filter = tasks_db.get_visible_tasks_filter(
        user_id=user_id,
        user_role=user_role,
...
async def get_leads_by_loan_type(
    ...
    leads = await leads_db.collection.find({
        "$or": [
            {"assigned_to": user_object_id},
            {"assigned_to": {"$in": [user_object_id]}}
        ],
        "loan_type": loan_type
    }).sort("created_at", -1).limit(100).to_list(length=100)  # Limit to 100 recent leads
...
async def get_leads_logins_by_type(
    ...
    raw_leads = await leads_db.collection.find(base_filter).sort("created_at", -1).limit(limit).to_list(length=limit)
...
async def get_all_leads_by_loan_type(
    ...
    leads_cursor = await leads_db.collection.find({
        "loan_type": loan_type
    ...
    try:
        ...
        try:
            ...
            if task_update.status and task_update.status != old_status:
                task_history_db.add_status_changed(task_id, user_id, user_name, old_status, task_update.status)
...
async def test_attachments_db(
    ...
    try:
        ...
        total_attachments = tasks_db.attachments_collection.count_documents({})
        total_tasks = await tasks_db.collection.count_documents({})
        
        # Get a sample of attachments
        sample_attachments = list(tasks_db.attachments_collection.find({}).limit(5))
...
async def get_task_history(
    task_id: str,
    user_id: str = Query(..., description="ID of the user requesting the history"),
    tasks_db: TasksDB = Depends(get_tasks_db),
    users_db: UsersDB = Depends(get_users_db),
    roles_db: RolesDB = Depends(get_roles_db)
):
    ...
    try:
        ...
        # Check user permissions using the same pattern as GET endpoint
        user = await users_db.get_user(user_id)
        if not user:
            print(f"User {user_id} not found")
            raise HTTPException(
        ...
        if not accessible_task:
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="You don't have permission to view this task history"
            )
        
        # Convert task to dictionary format for processing
        task_dict = convert_object_id(task)
        
        # For now, return a simple history structure
        # In the future, this could be expanded to track actual task changes
        history_entries = [
            {
                "id": "1",
                "action": "Task Created",
                "description": f"Task was created with status: {task_dict.get('status', 'Pending')}",
                "created_at": task_dict.get("created_at", datetime.now()),
                "created_by_name": task_dict.get("creator_name", "Unknown"),
                "created_by": task_dict.get("created_by")
            }
        ]
        
        # Add task update history if updated_at is different from created_at
        if (task_dict.get("updated_at") and task_dict.get("created_at") and 
            task_dict.get("updated_at") != task_dict.get("created_at")):
            history_entries.append({
                "id": "2",
                "action": "Task Updated",
                "description": f"Task was updated. Current status: {task_dict.get('status', 'Pending')}",
                "created_at": task_dict.get("updated_at", datetime.now()),
                "created_by_name": "System",
                "created_by": task_dict.get("created_by")
            })
        
        print(f"Successfully retrieved history for task {task_id}")
...
async def add_task_comment(
    ...
    try:
        ...
        try:
            task_history_db.add_comment_added(task_id, user_id, user_name, content.strip())
        except Exception as e:
