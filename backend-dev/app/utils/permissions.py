...
class PermissionManager:
    ...
    async def get_subordinate_users(
        ...
        if not subordinate_roles:
            ...
            # ── Peer Fallback: if no hierarchy configured, use same-role users as the "team".
            # This ensures view_team permission always works based on the permission grant alone,
            # regardless of whether a role hierarchy has been set up.
            peer_users = await users_db.get_users_by_roles([str(user_role_id)])
            peer_ids = [str(p["_id"]) for p in (peer_users or []) if str(p["_id"]) != str(user_id)]
            print(f"DEBUG: Peer fallback - found {len(peer_ids)} same-role users as team for {user_id}")
            return peer_ids
    ...
    async def can_view_lead(
        ...
        # Check for all/view_all permission (supports both old and new action names)
        ...
        has_view_all = any(
            (perm.get("page", "").lower() in _CV_LEADS_PAGES and 
             (perm.get("actions") == "*" or 
              ...
              (isinstance(perm.get("actions"), list) and 
               ("*" in perm.get("actions") or 
                any(a.lower() in ("all", "view_all") if isinstance(a, str) else False for a in perm.get("actions", []))))))
            for perm in user_permissions
        )
        ...
        # "show" permission alone only grants sidebar visibility, NOT data access.
        # Without own/junior/all, user can only see leads they created or are assigned to.
        ...
        # Check for junior/view_team permission (supports both old and new action names)
        has_view_junior = any(
            (perm.get("page", "").lower() in _CV_LEADS_PAGES and 
             (perm.get("actions") == "*" or 
              (isinstance(perm.get("actions"), str) and perm.get("actions").lower() == "view_team") or
    ...
    async def get_lead_visibility_filter(
        ...
        for perm in user_permissions:
            ...
            if page_matches:
                ...
                # Check for string 'all' or 'view_all' permission
                if isinstance(actions, str) and actions.lower() in ("all", "view_all"):
                    print(f"DEBUG: Found string '{actions}' permission")
                    has_view_all = True
                    break
                
                # Check for 'all'/'view_all' in array
                if isinstance(actions, list):
                    if "*" in actions:
                    ...
                    if any(action.lower() in ("all", "view_all") if isinstance(action, str) else False for action in actions):
                        print(f"DEBUG: Found 'all'/'view_all' in actions array")
                        has_view_all = True
                        break
        ...
        is_team_leader_by_permission = any(
            (perm.get("page", "").lower() in LEADS_PAGES_LOWER and 
             (perm.get("actions") == "*" or 
              ...
              (isinstance(perm.get("actions"), list) and 
               any(isinstance(a, str) and a.lower() in VIEW_ALL_ACTIONS | VIEW_TEAM_ACTIONS
        ...
        # Check for junior/view_team permission (case-insensitive)
        # Split into two: main leads page (all team leads) vs pl_odd_leads only (PL/OD leads from team)
        VIEW_TEAM_ACTIONS_SET = ("view_team",)

        def _has_view_team_action(perm):
            actions = perm.get("actions", [])
            if actions == "*":
                return True
            if isinstance(actions, str):
                return actions.lower() in VIEW_TEAM_ACTIONS_SET
            if isinstance(actions, list):
                return "*" in actions or any(
                    a.lower() in VIEW_TEAM_ACTIONS_SET if isinstance(a, str) else False
                    for a in actions
                )
            return False

        # view_team on the MAIN "leads" page → full team visibility (all lead types)
        has_view_junior = any(
            perm.get("page", "").lower() == "leads" and _has_view_team_action(perm)
            for perm in user_permissions
        )

        # view_team ONLY on pl_odd_leads (not on main leads page) → PL/OD leads from team only
        has_view_junior_plod_only = (not has_view_junior) and any(
            perm.get("page", "").lower() in {"leads.pl_odd_leads", "leads.pl_&_odd_leads"} and _has_view_team_action(perm)
            for perm in user_permissions
        )

        if has_view_junior or has_view_junior_plod_only:
            print(f"DEBUG: User {user_id} has junior permission (plod_only={has_view_junior_plod_only}) - checking subordinate users")
            ...
            if subordinate_users:
                ...
                print(f"DEBUG: Found {len(subordinate_list)} team users for {user_id}: {subordinate_list}")
                ...
                if has_view_junior_plod_only:
                    # PL/OD leads from team ONLY (file_sent_to_login = True)
                    print(f"DEBUG: Restricting team visibility to PL/OD leads (file_sent_to_login=True)")
                    plod_filter = {"file_sent_to_login": True}
                    filter_conditions.append({"$and": [plod_filter, {"created_by": {"$in": subordinate_list}}]})
                    filter_conditions.append({"$and": [plod_filter, {"assigned_to": {"$in": subordinate_list}}]})
                    filter_conditions.append({"$and": [plod_filter, {"assign_report_to": {"$in": subordinate_list}}]})
                    if subordinate_object_ids:
                        filter_conditions.append({"$and": [plod_filter, {"created_by": {"$in": subordinate_object_ids}}]})
                        filter_conditions.append({"$and": [plod_filter, {"assigned_to": {"$in": subordinate_object_ids}}]})
                        filter_conditions.append({"$and": [plod_filter, {"assign_report_to": {"$in": subordinate_object_ids}}]})
                else:
                    # Full team visibility — all lead types from team users
                    for sub_id in subordinate_list:
                    ...
                    filter_conditions.append({"assign_report_to": {"$in": subordinate_list}})
                    if subordinate_object_ids:
                ...
                print(f"DEBUG: Added team lead visibility filters for user {user_id}")
            else:
                print(f"DEBUG: No team users found for user {user_id} even after peer fallback")
                ...
                if team_members:
                    ...
                    if department_id:
                        ...
                        if subordinate_users:
                            ...
                            # Add subordinate assigned_to and assign_report_to (string format)
                            filter_conditions.append({"assigned_to": {"$in": subordinate_list}})
                            filter_conditions.append({"assign_report_to": {"$in": subordinate_list}})
    ...
    async def get_login_visibility_filter(
        ...
        # Check for all/view_all permission for LOGIN section
        has_view_all = False
        ...
        # Check for LOGIN junior/view_team permissions (supports both old and new action names)
    ...
    async def get_lead_user_capabilities(
        ...
        # Check for subordinate access (supports both old "junior" and new "view_team" action names)
        _GLUC_LEADS_PAGES = {"leads", "leads.pl_odd_leads", "leads.pl_&_odd_leads"}
        has_view_junior = any(
            (perm.get("page", "").lower() in _GLUC_LEADS_PAGES and 
             (perm.get("actions") == "*" or 
              (isinstance(perm.get("actions"), str) and perm.get("actions").lower() == "view_team") or
              (isinstance(perm.get("actions"), list) and 
               ("*" in perm.get("actions") or 
