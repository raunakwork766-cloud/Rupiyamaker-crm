...
async def list_employees(
    ...
    # Get user permissions to determine visibility scope
    ...
    # Check view_all / all permission on employees page
    has_view_all = is_super_admin or any(
        perm.get("page", "").lower() == "employees" and (
            perm.get("actions") == "*" or
            (isinstance(perm.get("actions"), str) and perm.get("actions").lower() in ("all", "view_all")) or
            (isinstance(perm.get("actions"), list) and any(
                a.lower() in ("all", "view_all", "*") if isinstance(a, str) else False
    ...
    # Check view_team / junior permission on employees page
    has_view_team = (not has_view_all) and any(
        perm.get("page", "").lower() == "employees" and (
            perm.get("actions") == "*" or
            (isinstance(perm.get("actions"), str) and perm.get("actions").lower() in ("junior", "view_team")) or
            (isinstance(perm.get("actions"), list) and any(
                a.lower() in ("junior", "view_team") if isinstance(a, str) else False
                for a in perm.get("actions", [])
            ))
        )
        for perm in user_permissions
    ...
    if has_view_all:
        ...
        # Can only see own subordinate employees
        subordinate_ids = await PermissionManager.get_subordinate_users(user_id, users_db, roles_db)
        subordinate_ids_set = set(subordinate_ids) | {user_id}  # include self
        all_employees = await users_db.get_employees_with_readable_passwords(status, department_id)
        employees = [e for e in all_employees if str(e.get("_id", "")) in subordinate_ids_set]
    else:
        # own only — just return the user themselves (if they are an employee)
        all_employees = await users_db.get_employees_with_readable_passwords(status, department_id)
        employees = [e for e in all_employees if str(e.get("_id", "")) == user_id]
