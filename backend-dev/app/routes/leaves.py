...
async def list_leaves(
    ...
    try:
        ...
        if can_see_all_leaves:
            ...
            # Check for view_team permissions (can see subordinates)
            has_junior = False
            for perm in permissions:
                if perm.get("page") == "leaves" and \
                   (isinstance(perm.get("actions"), list) and ("view_team" in perm.get("actions") or "view_team" in perm.get("actions"))) or \
                   perm.get("actions") in ("view_team", "junior"):
                    has_junior = True
                    break
