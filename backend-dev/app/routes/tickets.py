            ...
            if isinstance(perm, dict):
                ...
                if page in ["tickets", "ticket"]:
                    ...
                    if "view_team" in actions:
                        has_junior = True
                    if "all" in actions or "view_all" in actions:
                        has_all = True
...
async def list_tickets(
    ...
    try:
        ...
        # Check if user has junior/view_team permission - only then fetch subordinates
        has_junior_perm = False
        for perm in permissions:
            if isinstance(perm, dict) and perm.get("page") in ("tickets", "ticket"):
                actions = perm.get("actions", [])
                if isinstance(actions, str):
                    actions = [actions]
                if "view_team" in actions:
                    has_junior_perm = True
                    break
...
async def delete_ticket(
    ...
    try:
        ...
        # Also check if user has view_team (edit) permission as a fallback
