...
async def get_hierarchical_permissions(user_id: str, module: str) -> Dict[str, str]:
    ...
    try:
        ...
        # Check module-specific "view_team" permission (page="module" with actions="view_team")
        has_junior_permission = any(
            perm.get("page") == module and 
            (perm.get("actions") in ("view_team", "junior") or
             (isinstance(perm.get("actions"), list) and "view_team" in perm.get("actions", []) or "junior" in perm.get("actions", []) or "junior" in perm.get("actions", [])))
            for perm in permissions
        )
...
async def get_user_warnings(
    ...
    try:
        ...
        # Check if requesting user has view_team permission for hierarchical access
        has_view_junior = any(
            perm.get("page") == "warnings" and 
            (perm.get("actions") in ("view_team", "junior") or
             (isinstance(perm.get("actions"), list) and "view_team" in perm.get("actions", []) or "junior" in perm.get("actions", []) or "junior" in perm.get("actions", [])))
