...
async def get_interview_visibility_filter(
    ...
    try:
        ...
        # Check for view_team/junior permission (only view_team or junior triggers team data)
        has_view_junior = any(
            (perm.get("page", "").lower() == "interview" and 
             (perm.get("actions") == "*" or 
              (isinstance(perm.get("actions"), str) and perm.get("actions").lower() in ("view_team", "junior")) or
              (isinstance(perm.get("actions"), list) and 
               ("*" in perm.get("actions") or 
                any(action.lower() in ("view_team", "junior") if isinstance(action, str) else False 
                    for action in perm.get("actions", []))))))
            for perm in user_permissions
        )
