...
class TasksDB:
    ...
    async def get_visible_tasks_filter(self,
        ...
        # Check for view_team permission - includes "all" permissions
        has_view_junior = any(
            (perm.get("page") in ["tasks", "*", "any"] and 
             (perm.get("actions") == "*" or 
              ...
              (isinstance(perm.get("actions"), list) and 
               ("*" in perm.get("actions") or "view_team" in perm.get("actions") or "all" in perm.get("actions") or "view_all" in perm.get("actions")))))
            for perm in user_permissions
        )
